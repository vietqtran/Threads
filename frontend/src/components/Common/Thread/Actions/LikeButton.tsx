...
const LikeButton = ({ threadId }: Props) => {
    const [liked, setLiked] = React.useState(false)

    return (
        <div
            onClick={() => setLiked(!liked)}
            className="group relative flex h-9 min-w-9 cursor-pointer items-center justify-center gap-1 px-2 duration-75 ease-linear active:scale-90"
        >
            ...
            {liked ? (
                <Icon name="liked_thread" size={20} />
            ) : (
                <>
